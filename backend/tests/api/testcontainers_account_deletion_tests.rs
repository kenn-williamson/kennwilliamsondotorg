...
async fn test_user_can_delete_own_account() {
    ...
    let user = ctx
        .create_verified_user("test@example.com", "testuser")
        .await;
    let user_id = user.id;
    ...
    let mut resp = ctx
        ...
        .insert_header((
            "Authorization",
            format!("Bearer {}", create_test_jwt(&ctx, user_id)),
        ))
        .send()
        .await
        .unwrap();
...
async fn test_user_cannot_delete_other_users_account() {
    ...
    let resp = ctx
        ...
        .insert_header((
            "Authorization",
            format!("Bearer {}", create_test_jwt(&ctx, user1.id)),
        ))
        .send()
        .await
        .unwrap();
...
async fn test_user_phrase_suggestions_deleted() {
    ...
    let suggestion_count =
        sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM phrase_suggestions WHERE user_id = $1")
            .bind(user_id)
            .fetch_one(&ctx.pool)
            .await
            .unwrap();
    ...
    assert_eq!(
        suggestion_count, 0,
        "All phrase suggestions should be deleted"
    );
}
...
async fn test_system_user_cannot_be_deleted() {
    ...
    let mut resp = ctx
        ...
        .insert_header((
            "Authorization",
            format!("Bearer {}", create_test_jwt(&ctx, system_user_id)),
        ))
        .send()
        .await
        .unwrap();
...
async fn get_system_user_id(ctx: &TestContext) -> Uuid {
    sqlx::query_scalar::<_, Uuid>("SELECT id FROM users WHERE email = 'system@kennwilliamson.org'")
        .fetch_one(&ctx.pool)
        .await
        .unwrap()
}
...
async fn verify_user_data_deleted(ctx: &TestContext, user_id: Uuid) {
    ...
    let timer_count =
        sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM incident_timers WHERE user_id = $1")
            .bind(user_id)
            .fetch_one(&ctx.pool)
            .await
            .unwrap();
    ...
    assert_eq!(
        exclusion_count, 0,
        "All phrase exclusions should be deleted"
    );
...
async fn verify_phrases_reassigned(ctx: &TestContext, user_id: Uuid) {
    ...
    let user_phrase_count =
        sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM phrases WHERE created_by = $1")
            .bind(user_id)
            .fetch_one(&ctx.pool)
            .await
            .unwrap();
    assert_eq!(
        user_phrase_count, 0,
        "No phrases should be owned by deleted user"
    );

    // Check that phrases exist and are owned by system user
    let system_phrase_count =
        sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM phrases WHERE created_by = $1")
            .bind(system_user_id)
            .fetch_one(&ctx.pool)
            .await
