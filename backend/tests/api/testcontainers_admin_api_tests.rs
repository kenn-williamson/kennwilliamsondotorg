...
async fn test_admin_endpoints_require_authentication() {
    ...
    let mut resp = ctx
        .server
        .get("/backend/protected/admin/stats")
        .send()
        .await
        .unwrap();
    ...
    let mut resp = ctx
        .server
        .get("/backend/protected/admin/users")
        .send()
        .await
        .unwrap();
    assert_eq!(resp.status(), 401); // Unauthorized
...
async fn test_get_system_stats_success() {
    ...
    let token = crate::fixtures::create_test_jwt_token(&admin_user)
        .await
        .unwrap();
...
async fn test_get_users_with_search() {
    ...
    let mut resp = ctx
        .server
        .get("/backend/protected/admin/users?search=Admin")
        .insert_header(("Authorization", format!("Bearer {}", token)))
        .send()
        .await
        .unwrap();
...
async fn test_deactivate_user_success() {
    ...
    let mut resp = ctx
        .server
        .post(format!(
            "/backend/protected/admin/users/{}/deactivate",
            regular_user.id
        ))
        .insert_header(("Authorization", format!("Bearer {}", token)))
        .send()
...
async fn test_activate_user_success() {
    ...
    let mut resp = ctx
        .server
        .post(format!(
            "/backend/protected/admin/users/{}/activate",
            regular_user.id
        ))
        .insert_header(("Authorization", format!("Bearer {}", token)))
        .send()
...
async fn test_reset_user_password_success() {
    ...
    let mut resp = ctx
        .server
        .post(format!(
            "/backend/protected/admin/users/{}/reset-password",
            regular_user.id
        ))
        .insert_header(("Authorization", format!("Bearer {}", token)))
        .send()
...
async fn test_promote_user_to_admin_success() {
    ...
    let mut resp = ctx
        .server
        .post(format!(
            "/backend/protected/admin/users/{}/promote",
            regular_user.id
        ))
        .insert_header(("Authorization", format!("Bearer {}", token)))
        .send()
...
async fn test_get_all_phrases_success() {
    ...
    let mut resp = ctx
        .server
        .get("/backend/protected/admin/phrases")
        .insert_header(("Authorization", format!("Bearer {}", token)))
        .send()
        .await
        .unwrap();
...
async fn test_create_phrase_success() {
    ...
    let mut resp = ctx
        .server
        .post("/backend/protected/admin/phrases")
        .insert_header(("Authorization", format!("Bearer {}", token)))
...
async fn test_get_pending_suggestions_success() {
    ...
    let mut resp = ctx
        .server
        .get("/backend/protected/admin/suggestions")
        .insert_header(("Authorization", format!("Bearer {}", token)))
        .send()
        .await
        .unwrap();
...
async fn test_deactivate_nonexistent_user() {
    ...
    println!(
        "Deactivate nonexistent user response status: {}",
        resp.status()
    );
    if resp.status() != 404 {
        let body: serde_json::Value = resp.json().await.unwrap();
...
async fn test_invalid_user_id_format() {
    ...
    let mut resp = ctx
        .server
        .post("/backend/protected/admin/users/invalid-uuid/deactivate")
        .insert_header(("Authorization", format!("Bearer {}", token)))
        .send()
        .await
        .unwrap();
