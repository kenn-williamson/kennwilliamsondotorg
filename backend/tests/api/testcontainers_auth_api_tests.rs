...
async fn test_register_success() {
    ...
    assert_eq!(
        ...
        request_body.get("email").unwrap()
    );
    assert_eq!(
        user.get("display_name").unwrap(),
        request_body.get("display_name").unwrap()
    );
}
...
async fn test_login_success() {
    ...
    assert_eq!(
        returned_user.get("display_name").unwrap().as_str().unwrap(),
        display_name
    );
}
...
async fn test_get_current_user_success() {
    ...
    assert_eq!(
        body.get("display_name").unwrap().as_str().unwrap(),
        display_name
    );
}
...
async fn test_change_password_success() {
    ...
    let mut resp = ctx
        .server
        .put("/backend/protected/auth/change-password")
        .insert_header(("Authorization", format!("Bearer {}", token)))
        .send_json(&request_body)
        .await
        .unwrap();
...
async fn test_change_password_wrong_current() {
    ...
    println!(
        "Change password wrong current response status: {}",
        resp.status()
    );
    if resp.status() != 400 {
        let body: serde_json::Value = resp.json().await.unwrap();
...
async fn test_verify_email_success() {
    ...
    let mut resp = ctx
        .server
        .get(format!(
            "/backend/public/auth/verify-email?token={}",
            verification_token
        ))
        .send()
        .await
...
async fn test_verify_email_invalid_token() {
    ...
    let mut resp = ctx
        .server
        .get("/backend/public/auth/verify-email?token=invalid_token_123")
        .send()
        .await
        .unwrap();
...
async fn test_send_verification_email_unauthorized() {
    ...
    println!(
        "Unauthorized send verification response status: {}",
        resp.status()
    );
    assert_eq!(resp.status(), 401); // Unauthorized
}
