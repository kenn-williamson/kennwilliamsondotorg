...
async fn test_get_random_phrase_success() {
    ...
    let mut resp = ctx
        .server
        .get("/backend/protected/phrases/random")
...
async fn test_get_user_phrases_success() {
    ...
    let mut resp = ctx
        .server
        .get("/backend/protected/phrases/user")
        .insert_header(("Authorization", format!("Bearer {}", token)))
        .send()
        .await
        .unwrap();
...
async fn test_exclude_phrase_success() {
    ...
    let mut resp = ctx
        .server
        .post(format!("/backend/protected/phrases/exclude/{}", phrase_id))
        .insert_header(("Authorization", format!("Bearer {}", token)))
        .send()
        .await
        .unwrap();
...
async fn test_remove_phrase_exclusion_success() {
    ...
    let register_body: serde_json::Value = register_resp.json().await.unwrap();
    let token = register_body.get("token").unwrap().as_str().unwrap();
    let _user_id: uuid::Uuid = register_body
        .get("user")
        .unwrap()
        .get("id")
        .unwrap()
    ...
    let mut resp = ctx
        .server
        .delete(format!("/backend/protected/phrases/exclude/{}", phrase_id))
        .insert_header(("Authorization", format!("Bearer {}", token)))
        .send()
        .await
        .unwrap();
...
async fn test_submit_suggestion_success() {
    ...
    assert_eq!(
        body.get("phrase_text").unwrap(),
        "This is a test phrase suggestion"
    );
...
async fn test_get_public_phrase_success() {
    ...
    let mut resp = ctx
        .server
        .get(format!("/backend/public/{}/phrase", slug))
        .send()
        .await
        .unwrap();
...
async fn test_get_public_phrase_nonexistent_user() {
    ...
    let mut resp = ctx
        .server
        .get("/backend/public/nonexistent-user/phrase")
        .send()
        .await
        .unwrap();

    println!(
        "Get public phrase nonexistent user response status: {}",
        resp.status()
    );
    if !resp.status().is_success() {
        let body: serde_json::Value = resp.json().await.unwrap();
