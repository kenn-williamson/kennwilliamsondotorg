...
use backend::middleware::rate_limiter::{RateLimitServiceTrait, RedisRateLimitService};
use testcontainers::{
    GenericImage,
...
async fn test_redis_rate_limiting_integration() {
    ...
    let should_limit = rate_limit_service
        .check_rate_limit(identifier, endpoint, &config)
        .await
        .unwrap();
    ...
    // Increment counter
    rate_limit_service
        .increment_rate_limit(identifier, endpoint, &config)
        .await
        .unwrap();
    ...
    assert!(
        should_limit,
        "Second request should be limited by burst limit"
    );

    // Test with different identifier - should not be limited
    let different_identifier = "different_user";
    let should_limit = rate_limit_service
        .check_rate_limit(different_identifier, endpoint, &config)
        .await
        .unwrap();
