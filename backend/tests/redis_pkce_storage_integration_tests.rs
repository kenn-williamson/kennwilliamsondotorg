...
async fn test_pkce_storage_store_and_retrieve() {
    ...
    // Create PKCE storage
    let pkce_storage = RedisPkceStorage::new(&redis_url).expect("Failed to create PKCE storage");
    ...
    let second_retrieval = pkce_storage
        .retrieve_and_delete_pkce(state)
        .await
        .expect("Failed to retrieve PKCE verifier");
...
async fn test_pkce_storage_not_found() {
    ...
    let retrieved = pkce_storage
        .retrieve_and_delete_pkce("nonexistent-state")
        .await
        .expect("Failed to retrieve PKCE verifier");
...
async fn test_pkce_storage_overwrites_existing_state() {
    ...
    pkce_storage
        .store_pkce(state, "first-verifier", 300)
        .await
        .expect("Failed to store first verifier");

    // Overwrite with second verifier
    pkce_storage
        .store_pkce(state, "second-verifier", 300)
        .await
        .expect("Failed to store second verifier");
