...
async fn test_export_data_isolation() {
    ...
    let token1 = crate::fixtures::create_test_jwt_token(&user1)
        .await
        .unwrap();
    let token2 = crate::fixtures::create_test_jwt_token(&user2)
        .await
        .unwrap();
...
async fn test_export_data_empty_user() {
    ...
    assert_eq!(
        export_data["phrase_suggestions"].as_array().unwrap().len(),
        0
    );
    assert_eq!(
        export_data["phrase_exclusions"].as_array().unwrap().len(),
        0
    );
    assert_eq!(export_data["active_sessions"].as_array().unwrap().len(), 0);
    assert_eq!(
        export_data["verification_history"]
            .as_array()
            .unwrap()
            .len(),
        0
