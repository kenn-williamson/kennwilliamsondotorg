  ...
  <div class="users-tab">
    ...
    <div v-else-if="adminStore.users.length > 0" class="space-y-4">
      ...
      >
        <div class="flex items-center justify-between">
          <div class="flex-1">
            <div class="flex items-center space-x-3">
              <div class="flex-shrink-0">
                <div class="w-10 h-10 bg-nautical-100 rounded-full flex items-center justify-center">
                  <span class="text-nautical-600 font-medium text-sm">
                    {{ user.display_name.charAt(0).toUpperCase() }}
                  </span>
                </div>
              </div>
              <div class="flex-1 min-w-0">
                <h3 class="text-lg font-medium text-nautical-900 truncate">
                  {{ user.display_name }}
                </h3>
                <p class="text-sm text-nautical-500 truncate">{{ user.email }}</p>
                <p class="text-xs text-nautical-400">@{{ user.slug }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="flex items-center space-x-4">
            ...
            <div class="flex items-center space-x-2">
              <button
                @click="viewUser(user)"
                class="text-nautical-400 hover:text-nautical-600 transition-colors"
                title="View details"
              >
        ...
        <div v-if="adminStore.selectedUser?.id === user.id" class="mt-4 pt-4 border-t border-nautical-200">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <h4 class="text-sm font-medium text-nautical-900 mb-2">Account Information</h4>
              <dl class="space-y-1">
                <div>
                  <dt class="text-xs text-nautical-500">User ID</dt>
                  <dd class="text-sm text-nautical-900 font-mono">{{ user.id }}</dd>
                </div>
                <div>
                  <dt class="text-xs text-nautical-500">Created</dt>
                  <dd class="text-sm text-nautical-900">{{ formatDate(user.created_at) }}</dd>
                </div>
                <div>
                  <dt class="text-xs text-nautical-500">Roles</dt>
                  <dd class="text-sm text-nautical-900">{{ user.roles.join(', ') }}</dd>
                </div>
              </dl>
            </div>
            
            <div>
              <h4 class="text-sm font-medium text-nautical-900 mb-2">Roles</h4>
              <div class="space-y-2 mb-4">
                <!-- User role (base, immutable) -->
                <div class="flex items-center justify-between p-2 bg-nautical-50 rounded-md">
                  <div class="flex items-center">
                    <input
                      type="checkbox"
                      :checked="user.roles.includes('user')"
                      disabled
                      class="h-4 w-4 rounded border-nautical-300 text-nautical-400 cursor-not-allowed"
                    />
                    <label class="ml-2 text-sm text-nautical-700">
                      user <span class="text-xs text-nautical-500">(base role)</span>
                    </label>
                  </div>
                </div>

                <!-- Email Verified role (manageable) -->
                <div class="flex items-center justify-between p-2 bg-nautical-50 rounded-md hover:bg-nautical-100">
                  <div class="flex items-center">
                    <input
                      type="checkbox"
                      :checked="user.roles.includes('email-verified')"
                      @change="toggleRole(user, 'email-verified')"
                      class="h-4 w-4 rounded border-nautical-300 text-primary-600 focus:ring-blue-500"
                    />
                    <label class="ml-2 text-sm text-nautical-700 cursor-pointer">
                      email-verified
                    </label>
                  </div>
                </div>
                ...
                <div class="flex items-center justify-between p-2 bg-nautical-50 rounded-md hover:bg-nautical-100">
                  <div class="flex items-center">
                    <input
                      ...
                      class="h-4 w-4 rounded border-nautical-300 text-gold-600 focus:ring-amber-500"
                    />
                    <label class="ml-2 text-sm text-nautical-700 cursor-pointer">
                      trusted-contact
                    </label>
              ...
              <h4 class="text-sm font-medium text-nautical-900 mb-2 mt-4">Actions</h4>
              <div class="space-y-2">
                <button
                ...
                <button
                  ...
                  class="w-full px-3 py-2 text-sm bg-gold-100 text-gold-700 rounded-md hover:bg-gold-200 transition-colors"
                >
                  Reset Password
                </button>
              </div>
    ...
    <div v-else class="text-center py-12">
      ...
      <h3 class="text-lg font-medium text-nautical-900 mb-2">
        {{ adminStore.searchQuery ? 'No Users Found' : 'No Users Available' }}
      </h3>
      <p class="text-nautical-500 mb-4">
    ...
    <div v-if="adminStore.newPassword" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg shadow-xl max-w-md w-full mx-4 p-6">
        <h3 class="text-lg font-medium text-nautical-900 mb-4">Password Reset</h3>
        <p class="text-sm text-nautical-600 mb-4">
          New password for <strong>{{ adminStore.selectedUser?.display_name }}</strong>:
        </p>
        <div class="bg-nautical-100 rounded-md p-3 mb-4">
          <code class="text-sm font-mono text-nautical-900">{{ adminStore.newPassword }}</code>
        </div>
        <p class="text-xs text-nautical-500 mb-4">
        ...
        <div class="flex justify-end space-x-3">
          ...
          <button
            ...
            class="px-4 py-2 text-sm bg-nautical-900 text-white rounded-md hover:bg-nautical-800 transition-colors"
          >
            Close
          </button>
        </div>
