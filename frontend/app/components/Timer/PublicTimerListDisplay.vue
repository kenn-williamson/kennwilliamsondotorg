<template>
  <div class="public-timer-list-display">
    ...
    <div v-if="incidentTimerStore.publicTimersLoading" class="text-center py-8">
      ...
      <span class="mt-3 text-gray-600">Loading public timers...</span>
    </div>

    <!-- Empty State -->
    <div v-else-if="incidentTimerStore.publicTimersList.length === 0" class="text-center py-16">
      <p class="text-gray-600">{{ searchQuery ? 'No timers found matching your search.' : 'No public timers available yet.' }}</p>
      <p class="text-sm text-gray-500 mt-2">{{ searchQuery ? 'Try a different search term.' : 'Be the first to share your timer publicly!' }}</p>
    </div>
    ...
    <div v-else class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      <NuxtLink
        ...
        class="bg-white/80 backdrop-blur-sm rounded-lg shadow-lg border border-blue-200 p-6 hover:shadow-xl hover:border-blue-300 transition-all duration-200"
      >
